...
describe('scan', () => {
  ...
  it('should throw an exception if there are no files', async () => {
    const expected = new Error(
      `Could not scan C/C++ project: Error: There were no files in the target directory that could be scanned. Check if the directory is empty or if an ignore policy is active.`,
    );
    try {
